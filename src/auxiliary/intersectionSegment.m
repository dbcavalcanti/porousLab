...
function [flagInt,pint,t12,t34] = intersectionSegment(segment1,segment2)
    ...
    % Discard intersection if the continuum edge is located to the
    % left or right of the horizontal bounding box of the first segment
    ...
    % or above of the bounding box of the first segment
    ...
    % Check if the second segment is above or on the right side of the first segment
    if (sign123 > 0.0) && (sign124 > 0.0)
        flagInt = false;
        pint    = [];
        t12     = [];
    ...
    % Check if the second segment is bellow or on the left side of the first segment
    if (sign123 < 0.0) && (sign124 < 0.0)
        flagInt = false;
        pint    = [];
        t12     = [];
